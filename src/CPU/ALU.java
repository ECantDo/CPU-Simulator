...
public class ALU {
	...
	public ALU(boolean equalsZero, boolean carryOut) {
	...
	private int add(int input_a, int input_b) {
		int res = input_a + input_b;
		this.carryOut = (res & 0x10000) != 0;
		this.equalsZero = res == 0;
		return res & CPUSpecs.bitMask;
	}

	private int addWithFlags(int input_a, int input_b) {
		int res = input_a + input_b + (carryOut ? 1 : 0);
		this.carryOut = (res & 0x10000) != 0;
		this.equalsZero = res == 0;
		return res & CPUSpecs.bitMask;
	}
	...
	private int arshift(int input_a, int input_b) {
		input_b = input_b & 0xF; // 16-bit, only able to shift 15 times
		if ((input_a & 8000) != 0)  // Only populate upper bits when the sign bit is 1
